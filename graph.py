"""Graph utilities."""
# modified to get weight in transformation
...
class Graph(object):
    ...
    def read_edgelist(self, filename, weighted=False, directed=False):
        ...
        if directed:
            ...
            def read_weighted(l):
                src, dst, w = l.split()
                self.G.add_edge(src, dst, weight=float(w))
                self.G[src][dst]['weight'] = float(w)
        else:
            def read_unweighted(l):
                src, dst = l.split()
                self.G.add_edge(src, dst, weight=1.0)
                self.G.add_edge(dst, src, weight=1.0)
                self.G[src][dst]['weight'] = 1.0
            ...
            def read_weighted(l):
                src, dst, w = l.split()
                self.G.add_edge(src, dst, weight=float(w))
                self.G.add_edge(dst, src, weight=float(w))
                self.G[src][dst]['weight'] = float(w)
